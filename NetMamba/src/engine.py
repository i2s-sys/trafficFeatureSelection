...
class ClassBalancedFocalLoss(torch.nn.Module):
    def __init__(self, class_weights, gamma, loss_type):
        """
        Args:
            class_weights: 字典，格式如 {'0': weight_0, '1': weight_1, ..., 'N': weight_N}
            gamma: Focal Loss 的 gamma 参数，默认 2.0
            loss_type: 选择 "ce"（普通交叉熵）/ "cb"（类平衡交叉熵）/ "cb_focal_loss"（类平衡 Focal Loss）
        """
        super(ClassBalancedFocalLoss, self).__init__()
        self.class_weights = torch.tensor([class_weights[str(i)] for i in range(len(class_weights))], dtype=torch.float32)
        self.gamma = gamma
    ...
    def forward(self, logits, targets):
        ...
        Returns:
            loss: 计算出的损失值
        """
        ce_loss = F.cross_entropy(logits, targets, reduction='none')  # 计算交叉熵损失

        # 计算类别权重
        class_weights = self.class_weights.to(logits.device)  # 发送到相同设备
        sample_weights = class_weights[targets]  # 选取对应类别的权重

        # 计算 CB-CE（类平衡交叉熵）
        cbce_loss = ce_loss * sample_weights

        # 计算 Focal Loss
        softmax_probs = F.softmax(logits, dim=1)  # 计算 softmax 概率
        targets_one_hot = F.one_hot(targets, num_classes=len(class_weights)).float()  # 转换为 one-hot
        pt = (targets_one_hot * softmax_probs).sum(dim=1)  # 计算 pt
        focal_modulator = (1.0 - pt) ** self.gamma  # Focal Loss modulator
        focal_loss = focal_modulator * ce_loss

        # 计算 CB-Focal Loss（类平衡 Focal Loss）
        cb_focal_loss = focal_loss * sample_weights

        # 选择最终的损失函数
        if self.loss_type == "ce":
            return ce_loss.mean()  # 普通交叉熵
        elif self.loss_type == "cb":
            return cbce_loss.mean()  # 类平衡交叉熵
        elif self.loss_type == "cb_focal_loss":
            return cb_focal_loss.mean()  # 类平衡 Focal Loss
        else:
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    data_loader: Iterable, optimizer: torch.optim.Optimizer,
                    device: torch.device, epoch: int, loss_scaler, amp_autocast, selection_factor=None, max_norm: float = 0,
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        # samples = samples * selection_factor.view(1, 1, 1, -1)

        if mixup_fn is not None:
            samples, targets = mixup_fn(samples, targets)
        # l1_norm = torch.norm(selection_factor, p=1)
        # lamuda = 0.001
        with amp_autocast():
            outputs = model(samples)
            loss = criterion(outputs, targets) # + lamuda * l1_norm
    ...
    # 输出选择因子的值
    # selection_factor_values = selection_factor.cpu().detach().numpy().round(4).tolist()
    # print(f"Selection factor after epoch {epoch}: {selection_factor_values}")
