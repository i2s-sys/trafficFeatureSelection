...
class AppScanner(object):
    ...
    def fit(self, X, y):
        """Fit model with given training data and labels. 用给定的训练数据和标签拟合模型。
    ...
    def fit_predict(self, X, y):
        """Fit model and return the prediction on the same data. 拟合模型并返回对相同数据的预测。
            ...
            result : np.array of shape=(n_samples,)
                Prediction of labels for data X.
                Labels are -1 if they cannot be predicted. 如果标签无法预测，则为-1。
            """
