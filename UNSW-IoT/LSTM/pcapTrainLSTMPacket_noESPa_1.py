...
import numpy as np
import csv, os
from pcapLSTMPacket_noESPa_1 import LSTM,LSTM2 # 导入DNN类
import warnings
...
# 训练模型
model.train_start = 0
for _ in range(EPOCH_NUM):  # 循环100次 # 调用train函数，并获取损失的变化
    delta_loss, count = model.train()  # 判断损失的变化是否小于阈值
    model.epoch_count += 1
    # if model.earlyStop == True:  # 50次epoch差距小 或者 早停达成退出训练
    #     print("model.earlyStop == True 继续查看变化情况")
    #     break
...
with model.sess as sess:
    ...
    for _ in range(EPOCH_NUM):  # 调用train函数，并获取损失的变化
        ...
        dnn2.epoch_count += 1
        if count >= 8:
            print("count >= 8: 连续10个epoch满足条件 变化不大 相差小于0.03")
            break
    end_time = time.time()  # 记录训练结束时间
    total_training_time = end_time - start_time  # 计算训练总时长
