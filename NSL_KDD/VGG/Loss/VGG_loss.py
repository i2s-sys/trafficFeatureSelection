...
MODEL_SAVE_PATH = '../FeatureSelect2/model/'
...
class VGG():
    def __init__(self,K,lossType,ES_THRESHOLD,seed,beta, gamma):  # 不输入维度 默认是DATA_DIM
    ...
    def create_VGG(self):
        ...
        x = tf.keras.layers.Conv2D(64, (3, 3), activation='relu', padding='same',
                                   name='conv_1',
                                   kernel_initializer=tf.keras.initializers.glorot_uniform(seed=self.seed))(scaled_input)
        x = tf.keras.layers.Conv2D(64, (3, 3), activation='relu', padding='same',
                                   name='conv_2',
    ...
    def train(self):
        ...
        if len(self.intersection_sets) >= self.ES_THRESHOLD:  # 仅当元素个数>0参与判定
