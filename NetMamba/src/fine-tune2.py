...
#20
    ...
    parser.add_argument('--output_dir', default=f'./output/finetune{curr_time}',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default=f'./output/finetune{curr_time}',
...
def main(args):
    ...
    print('job dir: {}'.format(os.path.dirname(os.path.realpath(__file__))), file=log)
    ...
    print("Sampler_train = %s" % str(sampler_train), file=log)
    if args.dist_eval:
        if len(dataset_val) % num_tasks != 0:
            print('Warning: Enabling distributed evaluation with an eval dataset not divisible by process number. '
                  'This will slightly alter validation results as extra duplicate entries are added to achieve '
                  'equal num of samples per-process.', file=log)
    ...
    if mixup_active:
        print("Mixup is activated!", file=log)
        mixup_fn = Mixup(
    ...
    if os.path.exists(args.finetune) and not args.eval:
        ...
        print("Load pre-trained checkpoint from: %s" % args.finetune, file=log)
        checkpoint_model = checkpoint['model']
        state_dict = model.state_dict()
        for k in ['head.weight', 'head.bias']:
            if k in checkpoint_model and checkpoint_model[k].shape != state_dict[k].shape:
                print(f"Removing key {k} from pretrained checkpoint", file=log)
    ...
    print("Model = %s" % str(model_without_ddp), file=log)
    print('number of params (M): %.2f' % (n_parameters / 1.e6), file=log)
    ...
    print("base lr: %.2e" % (args.lr * 256 / eff_batch_size), file=log)
    print("actual lr: %.2e" % args.lr, file=log)

    print("accumulate grad iterations: %d" % args.accum_iter, file=log)
    print("effective batch size: %d" % eff_batch_size, file=log)
    ...
    print("criterion = %s" % str(criterion), file=log)

    misc.load_model(args=args, model_without_ddp=model_without_ddp, optimizer=optimizer, loss_scaler=loss_scaler)

    if args.eval:
        test_stats = evaluate(data_loader_val, model, device)
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%", file=log)
        exit(0)

    print(f"Start training for {args.epochs} epochs", file=log)
    ...
    best_acc = 0
    best_epoch = 0
    selection_factor = torch.nn.Parameter(torch.ones(72, device=device))
    optimizer.add_param_group({'params': selection_factor})
    intersection_sets = []
    TSMRecord = []
    earlyStop = False
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            optimizer, device, epoch, loss_scaler, amp_autocast,selection_factor,
        ...
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.4f}", file=log)
        max_accuracy = max(max_accuracy, test_stats["acc1"])
        print(f"F1 of the network on the {len(dataset_val)} test images: {test_stats['weighted_f1']:.4f}", file=log)
        max_f1 = max(max_f1, test_stats["weighted_f1"])
        print(f'Max Accuracy: {max_accuracy:.4f}', file=log)
        print(f'Max F1: {max_f1:.4f}', file=log)
        ...
        def get_top_k_indices(tensor, k):
            ...
            return indices.cpu().numpy()  # 转换为 NumPy 数组

        keyFeatureNums = 16
        top_k_indices = get_top_k_indices(selection_factor, keyFeatureNums)  # 前k个因子索引
