...
from engine import train_one_epoch, evaluate, ClassBalancedFocalLoss
...
def get_args_parser():
    ...
    parser.add_argument('--lossType', type=str, default='ce', help='')
...
def main(args):
    ...
    def get_label_status(dataset):
        """
        统计数据集中每个类别的数量
        Args:
            dataset: PyTorch Dataset 实例，假设 `dataset[i]` 返回 (data, label)
        Returns:
            label_status: dict，形如 {'0': 样本数, '1': 样本数, ...}
        """
        labels = [label for _, label in dataset]  # 提取所有标签
        label_status = dict(Counter(labels))  # 统计每个类别的数量
        return label_status

    def compute_class_weights(label_status, beta):
        """
        计算类别平衡权重 (Class-Balanced Weight)
        Args:
            label_status: dict，形如 {'0': 样本数, '1': 样本数, ...}
            beta: 平滑因子，通常在 [0.9, 0.999] 之间
        Returns:
            class_weights: dict，形如 {'0': 权重, '1': 权重, ...}
        """
        ClassNum = len(label_status)  # 类别数
        effective_num = {key: (1.0 - beta) / (1.0 - np.power(beta, value)) for key, value in label_status.items()}
        total_effective_num = sum(effective_num.values())  # 计算有效样本总数
        class_weights = {key: effective_num[key] / total_effective_num * ClassNum for key in effective_num}
        return class_weights

    label_status = get_label_status(dataset_train)
    class_weights = compute_class_weights(label_status, beta=args.beta)

    # 打印结果
    print("Label Status:", label_status)
    ...
    if mixup_fn is not None:
        ...
        criterion = ClassBalancedFocalLoss(class_weights=class_weights, gamma=args.gamma, loss_type=args.lossType).to(device)
