...
# 暂时注释掉了早停的指标 先不早停 执行30轮
tf.compat.v1.disable_eager_execution()

DATA_DIM = 72
OUTPUT_DIM = 29
BETA = 0.999 # 类平衡损失的β gamma
GAMMA = 1
...
class Resnet():
    ...
    def train(self):
        ...
        print(
            f'Epoch {self.epoch_count + 1} completed, average loss: {average_loss}, duration: {epoch_duration:.2f} seconds')

        # 转换self.scaling_factor为只保留小数点后四位的数组
        scaling_factor = self.sess.run(self.scaling_factor)
        scaled_factor_rounded = np.round(scaling_factor, 4)
        self.scaling_factors_history.append(scaled_factor_rounded)  # 记录当前epoch的选择因子
        ...
        if len(self.intersection_sets) > self.ES_THRESHOLD:  # 保持列表长度为2就可以开始统计
            self.intersection_sets.pop(0)
        if len(self.intersection_sets) >= 1:
