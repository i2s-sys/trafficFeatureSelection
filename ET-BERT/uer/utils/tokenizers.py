...
class BertTokenizer(Tokenizer):
    ...
    def tokenize1(self, text):
        ...
        else: # 获取 text 的每一个token，再对token进行分词 看是否有sub_token 对于string udp other都是'unk' 146被拆成了14和##6 1.0原本就被拆分为1 . 和 0 .被归为unk
            split_tokens = []
            token_counter = 0
            for token in self.basic_tokenizer.tokenize(text):
                for sub_token in self.wordpiece_tokenizer.tokenize(token):
                    split_tokens.append(sub_token)
                    token_indices.append(token_counter)
                token_counter += 1
            # for token1 in self.basic_tokenizer.tokenize(text):
            #     for token in self.basic_tokenizer.tokenize1(token1):
            #         for sub_token in self.wordpiece_tokenizer.tokenize(token):
            #             split_tokens.append(sub_token)
            #             token_indices.append(token_counter)
            #     token_counter += 1
        return split_tokens, token_indices
...
class BasicTokenizer(object):
    ...
    def tokenize(self, text):
        ...
        #     split_tokens.extend(self._run_split_on_punc(token))  # 新策略是把这组特征对应一系列token
        #     # split_tokens.append(token) # 因为.会被_run_split_on_punc认为是标点符号 故去掉这个函数和extend

        # output_tokens = whitespace_tokenize(" ".join(split_tokens)) 本来是复杂的
        return orig_tokens

    def tokenize1(self, token):
        """第二次的tokenize 要将.分隔开来"""
        token = convert_to_unicode(token)
        split_tokens1 = []
        for token in token:
            split_tokens1.extend(self._run_split_on_punc(token))  # 新策略是把这组特征对应一系列token
        output_tokens = whitespace_tokenize(" ".join(split_tokens1))
        return output_tokens

    def _run_strip_accents(self, text):
        """Strips accents from a piece of text. 从一段文本中去掉重音。"""
    ...
    def _run_split_on_punc1(self, text):
        """Splits punctuation on a piece of text."""
        chars = list(text)
        i = 0
        start_new_word = True
