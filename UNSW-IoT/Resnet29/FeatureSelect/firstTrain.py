...
def top_k_indices_heap(values, k):
    """Return indices of top-k values using heap-based selection."""
    flat = values.flatten()
    top_k = heapq.nlargest(k, enumerate(flat), key=lambda x: x[1])
    return np.array([idx for idx, _ in top_k], dtype=np.int64)
...
sorted_indices = top_k_indices_heap(scaling_factor_value, K)
